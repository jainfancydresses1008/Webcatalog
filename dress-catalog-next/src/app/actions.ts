...
export async function createDress(formData: FormData) {
  ...
  if (!category || !characterName || !description || !mainImageUrl) {
    throw new Error(
      "Category, character name, description and image are required.",
    );
  }
  if (
    sizes.length === 0 ||
    sizes.length !== prices.length ||
    prices.some((price) => Number.isNaN(price))
  ) {
...
export async function replaceDressImage(formData: FormData) {
  ...
  const existingImage = await prisma.dressImage.findUnique({
    where: { id: imageId },
  });
  ...
  await deleteImageFromCloudinary(existingImage.url).catch((error) =>
    console.error("Cloudinary delete skipped:", error),
  );
...
export async function addGalleryImage(formData: FormData) {
  ...
  if (!dressId || !imageUrl)
    throw new Error("Dress id and image are required.");
...
export async function deleteDress(formData: FormData) {
  ...
  await Promise.all(
    dress.images.map((image) =>
      deleteImageFromCloudinary(image.url).catch(() => undefined),
    ),
  );
  revalidatePath("/");
  revalidatePath("/admin/manage");
