  ...
  function submit(formData: FormData) {
    ...
    startTransition(async () => {
      try {
        ...
        setMessage(e instanceof Error ? e.message : "Unable to add dress.");
      }
    });
  }
  return (
    ...
    >
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Upload a dress image to Cloudinary or paste an image URL. Sizes and
          prices must match by position.
        </p>
      </div>
      {message && (
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <input
          name="category"
          required
          placeholder="Category, example Princess"
          className="rounded-2xl border border-slate-300 p-3"
        />
        <input
          name="subcategory"
          placeholder="Subcategory, example Accesories/Hats"
          className="rounded-2xl border border-slate-300 p-3"
        />
        <input
          name="characterName"
          required
          placeholder="Character Name, example Elsa"
          className="rounded-2xl border border-slate-300 p-3"
        />
        <input
          name="imageUrl"
        ...
        <textarea
          name="galleryUrls"
          placeholder="Optional gallery image URLs, one URL per line"
          className="min-h-24 rounded-2xl border border-slate-300 p-3 md:col-span-2"
        />
      </div>
      <div className="mt-4 flex flex-col gap-3 md:flex-row">
        <button
          disabled={pending}
          type="submit"
          className="rounded-2xl bg-green-600 px-5 py-3 font-semibold text-white hover:bg-green-700 disabled:opacity-60"
        >
          {pending ? "Adding..." : "Add Dress to Catalog"}
        </button>
        <button
          type="reset"
