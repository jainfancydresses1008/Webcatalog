...
export default function AdminDressForm({ createDressAction }: Props) {
  ...
  return (
    <form ref={formRef} action={submit} className="rounded-3xl bg-white p-5 shadow-sm">
      <h2 className="text-2xl font-black text-slate-950">Add New Dress</h2>
      <p className="mt-1 text-sm text-slate-600">Upload a dress image to Cloudinary or paste an image URL.</p>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        <input name="sizes" defaultValue="S, M, L" required className="rounded-2xl border border-slate-300 p-3" />
        <input name="prices" defaultValue="1000, 1200, 1400" required className="rounded-2xl border border-slate-300 p-3" />
        <textarea name="galleryUrls" placeholder="Optional gallery image URLs, one per line" className="min-h-24 rounded-2xl border border-slate-300 p-3 md:col-span-2" />
      </div>
      <button disabled={isPending} type="submit" className="mt-4 rounded-2xl bg-green-600 px-5 py-3 font-bold text-white hover:bg-green-700 disabled:opacity-60">{isPending ? 'Adding...' : 'Add Dress'}</button>
    </form>
  );
}
