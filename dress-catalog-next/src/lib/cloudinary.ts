    ...
    const uploadStream = cloudinary.uploader.upload_stream(
      ...
      (error, result) => {
        if (error) {
          ...
          reject(
            new Error(
              `CLOUDINARY_UPLOAD_ERROR: ${error.message ?? JSON.stringify(error)}`,
            ),
          );
          return;
        }
        if (!result?.secure_url || !result.public_id)
          return reject(
            new Error(
              "Cloudinary upload did not return required asset metadata.",
            ),
          );
...
export async function uploadImageToCloudinary(file: File): Promise<string> {
  return (await uploadImageAssetToCloudinary(file)).secure_url;
}
...
export async function listCloudinaryImages() {
  const assets: CloudinaryImageAsset[] = [];
  ...
  do {
    ...
    assets.push(...(result.resources as CloudinaryImageAsset[]));
