...
async function main() {
  ...
  const dresses = [
    {
      category: "Princess",
      characterName: "Elsa",
      description: "Frozen blue gown with silver snowflake design",
      images: [
    ...
    },
    {
      ...
      description: "Classic red and blue superhero costume",
      images: [
    ...
    },
    {
      ...
      description: "Fun cartoon party dress for kids",
      images: [
  ...
  for (const item of dresses) {
    const category = await prisma.category.upsert({
      ...
      create: {
        name: item.category,
        description: `${item.category} fancy dresses for children.`,
      },
    });
      ...
      data: {
        ...
        images: {
          create: item.images.map((url, index) => ({
            url,
            altText: `${item.characterName} image ${index + 1}`,
            isMain: index === 0,
            sortOrder: index,
          })),
        },
